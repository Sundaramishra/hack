    ...
    switch ($method) {
        ...
        case 'POST':
            if ($action === 'add') {
                ...
                // Generate patient ID
                $patient_code = 'P' . date('Y') . str_pad(rand(1, 9999), 4, '0', STR_PAD_LEFT);
                ...
                try {
                    ...
                    $patient_query = "INSERT INTO patients (user_id, patient_id, date_of_birth, gender, 
                                    blood_group, emergency_contact_name, emergency_contact_phone, 
                                    assigned_doctor_id) 
                                    VALUES (:user_id, :patient_id, :date_of_birth, :gender, 
                                    :blood_group, :emergency_contact_name, :emergency_contact_phone, 
                                    :assigned_doctor_id)";
                    ...
                    // If doctor is assigned, create assignment record
                    if (!empty($data['assigned_doctor_id'])) {
                        $assignment_query = "INSERT INTO doctor_patient_assignments (doctor_id, patient_id, assigned_date, is_active) 
                                           VALUES (:doctor_id, :patient_id, NOW(), 1)";
                        $assignment_stmt = $conn->prepare($assignment_query);
                        $assignment_stmt->bindParam(':doctor_id', $data['assigned_doctor_id']);
                        $assignment_stmt->bindParam(':patient_id', $patient_id);
                        $assignment_stmt->execute();
                    }
                    
                    $conn->commit();
                    echo json_encode(['success' => true, 'message' => 'Patient added successfully', 'patient_id' => $patient_code]);
        ...
        case 'PUT':
            if ($action === 'update') {
                ...
                try {
                    ...
                    // Update patient info
                    $patient_query = "UPDATE patients SET date_of_birth = :date_of_birth, gender = :gender,
                                    blood_group = :blood_group, emergency_contact_name = :emergency_contact_name,
