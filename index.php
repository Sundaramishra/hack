...
<style>
  ...
  *{box-sizing:border-box}
  ...
  @media (max-width:600px){
    ...
    .expertise-list{width:90vw;max-width:90vw;margin:0 auto;padding:0;grid-template-columns:1fr;gap:1.2rem}
    .glass-card{padding:1.2rem;border-radius:12px;min-height:auto}
    .glass-card .title{font-size:1.4rem;margin-bottom:.5em}
    .glass-card ul{font-size:.9rem;line-height:1.6}
    .glass-card li{margin-bottom:.2em}
  }

  /* =============== BRANDS (Desktop: 4-3-4, Mobile: 2-2-2-2-2) =============== */
  .brands-section{
  ...
  /* Desktop brands layout: 4-3-4 pattern */
  @media (min-width:769px){
    .brands-desktop{display:block}
    .brands-mobile{display:none}
  }
  
  /* Mobile brands layout: 2-2-2-2-2 pattern */
  ...
  /* Mobile — bigger slider, less spacing */
  @media (max-width:700px){
    ...
    .featured-section{min-height:80vh;padding:10px 0}
    .featured-3d-heading{margin:15px 0 5px}
    .featured-3d-slider-track{height:60vh}
    .featured-3d-slide{width:200px;height:300px;border-width:2px;border-radius:12px}
    .featured-3d-arrow{width:50px;height:50px}
    .featured-3d-arrow.left{left:10px} .featured-3d-arrow.right{right:10px}
    .featured-caption{width:min(90vw,500px)}
    .featured-caption__title{font-size:clamp(1.2rem,6vw,1.8rem)}
    .featured-caption__sub{font-size:clamp(.9rem,3.5vw,1rem)}
  }
  @media (max-width:380px){
    :root{ --c-gap-1: 100px; --c-gap-2: 180px; }
...
<section class="brands-section section-overlap my-8" style="position:relative; margin-top:-3.8rem; z-index:11;">
  ...
  <div class="brands-grid">
    <?php
      ...
      if (!empty($brandLogos)) {
        // Desktop: 4-3-4 pattern
        $desktopPattern = [4,3,4];
        $k = 0;
        ...
        foreach($desktopPattern as $row) {
          ...
          for ($j = 0; $j < $row && $k < count($brandLogos); $j++, $k++) {
            $logo = $brandLogos[$k];
            echo '<img src="'.htmlspecialchars($logo['logo_path']).'" alt="Brand Logo" class="brands-logo">';
          }
          echo '</div>';
        }
        echo '</div>';
        
        // Mobile: 2-2-2-2-2 pattern (2 per row)
        $k = 0;
        echo '<div class="brands-mobile">';
        while ($k < count($brandLogos)) {
          echo '<div class="brands-row">';
          for ($j = 0; $j < 2 && $k < count($brandLogos); $j++, $k++) {
            $logo = $brandLogos[$k];
            echo '<img src="'.htmlspecialchars($logo['logo_path']).'" alt="Brand Logo" class="brands-logo">';
          }
          echo '</div>';
